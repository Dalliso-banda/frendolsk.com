...
export async function getAnalyticsSummary(
  ...
  const viewsByDay = await db('analytics_daily_stats')
    ...
    .groupBy('date')
    .select('date', db.raw('SUM(page_views) as views'), db.raw('SUM(unique_visitors) as visitors'))
    .orderBy('date', 'asc');
  ...
  const top404s = await db('analytics_page_views')
    ...
    .where('status_code', 404)
    .groupBy('page_path')
    .select('page_path as path', db.raw('COUNT(*) as hits'), db.raw('MAX(created_at) as last_hit'))
    .orderBy('hits', 'desc')
    .limit(10);
...
// =============================================================================
// Utility Exports (used by analytics.test.ts and other consumers)
// =============================================================================
